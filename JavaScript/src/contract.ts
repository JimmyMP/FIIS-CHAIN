...
class StarterContract {
  ...
  // Método de ESCRITURA para eliminar un participante
  //near call jimmymp.testnet eliminar_participante '{"cuenta":"jimmymp.testnet"}' --accountId jimmymp.testnet
@call({})
eliminar_participante({ cuenta }: { cuenta: string }): boolean {

    // Si la cuenta ejecutando el comando no es aklassen.testnet, no podrá hacerse ningún cambio.
    assert(near.signerAccountId() == "jimmymp.testnet", "No tienes permisos para ejecutar este comando.");

    // Buscamos al participante
    let participante = this.participantes.get(cuenta);

    // Verificamos si encontramos al participante y si no está certificado
    if (participante && !participante.certificado) {
        // Eliminamos al participante de la lista
        this.participantes.remove(cuenta);
        near.log("Participante eliminado exitosamente.");
        return true;
    } else {
        // Si el participante no existe o está certificado, regresamos false
        near.log("No se puede eliminar al participante.");
        return false;
    }
}
   ...
   */
  //near call jimmymp.testnet set_participante '{"nombre":"Isaac","edad":28}' --accountId jimmymp.testnet --amount 1.1 
   ...
   */
  //near view jimmymp.testnet get_participante '{"cuenta":"jimmymp.testnet"}'
  @view({})
   ...
   */
  //near view jimmymp.testnet get_participantes
  @view({})
  get_participantes() {
